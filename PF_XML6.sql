...
-- Преобразовать XML в BLOB (encoding="windows-1251")Function xml2blob
( p_xml XMLType
...
-- Опись пачки документов 
  select substr(nvl(nvl(onames,oname),nvl(onames9,oname9)),1,50), pf_xml7.pfn2num(nvl(opfn,opfn9)) 
...
begin 
  ...
  for ft in (select rownum i, ftbl.* from table(pf_xml7.forms_tbl()) ftbl where xid=p_xid) 
  loop 
  ...
  select '' xname 
    ...
    from  
      (select fid, isum, fsum, nvl(cisum, isum) cisum, nvl(cfsum, fsum) cfsum  
         from table(pf_xml7.forms_tbl) where xid=p_xid) ft 
      left join  
      (select fid 
         , sum(nvl(sumt,0)) sumt, sum(nvl(sumb,0)) sumb, sum(nvl(sumo,0)) sumo 
         from table(pf_xml7.incomes_tbl) where pmon between l_smon and l_emon 
         group by fid) it  
      on ft.fid=it.fid; 
*/   
  for ft in 
...
CREATE OR REPLACE PACKAGE  "PF_XML" as
-- Пакет анализа/генерации обменных XML-файлов ПФР
-- Размер полей и переменных varchar2 для кодировки UTF-8
-- 2014 miktim@mail.ru Петрозаводский государственный университет. РЦНИТ.
-- 2014.11.26
period_error EXCEPTION; -- ошибка данных периода
pragma exception_init(period_error,-20201);
org_error EXCEPTION;    -- ошибка данных организации
...
pragma exception_init(unsupported,-20204);
--
c_program constant varchar2(256):='PF_XML';
c_version constant varchar2(10):='1.0.41203';
-- Номер (вид) формы
fn_szv1 constant number(1) := 1; -- СЗВ-6-1 (2010-2012)
fn_szv2 constant number(1) := 2; -- СЗВ-6-2 (2010-2012, СЗВ-6-1 без льготных периодов)
fn_szv3 constant number(1) := 3; -- СЗВ-6-3 (2011-2012, не упоминается с 2014г)
fn_szv4 constant number(1) := 4; -- СЗВ-6-4 (2013)
fn_szvr constant number(1) := 5; -- СЗВ-РСВ (2014-? РСВ-1 6й раздел)
--
fn_adv constant number(1) := 8;  -- опись пачек форма АДВ
fn_rsv constant number(1) := 9;  -- расчетная форма РСВ
--
subType money_tp is number(15,2);
subType emptype_tp is varchar2(10); -- код категории застрахованного лица
-- ORG Плательщик
Type org_rc is record
( ONAME varchar2(100) -- краткое наименование организации
, OPFN number(12)   -- регистрационный номер организации в ПФР
, OINN number(12)   -- ИНН организации
, OKPP number(9)    -- КПП организации
, OKVED varchar2(32)-- ОКВЭД организации
, PINDEX number(1)  -- индекс отчетного периода (1-4)
, PYEAR number(4)   -- отчетный год
, SDECK number(5):=0 -- стартовый номер пачек   
);
-- HDR Заголовок формы (СЗВ-6-1,2,3,4, СЗВ-РСВ), таблица форм
Type hdr_rc is record
( FID number           -- индекс формы
--, EXTFID varchar2(128)  -- внешний ID формы
, FN number(1)         -- номер (вид) формы
, LNAME varchar2(80)   -- фамилия
, FNAME varchar2(80)   -- имя
, SNAME varchar2(80)   -- отчество
, SNILS number(11)     -- страховой номер индивидуального лицевого счета (СНИЛС)
, FTYPE varchar2(6)    -- тип сведений: ИСХ, КОР, ОТМ
, CPINDEX number(1)    -- индекс корректируемого периода (1-4 квартал)
, CPYEAR number(4)     -- корректируемый год (2011-?)
, COPFN number(12)     -- регистрационный номер организации в ПФР в корректируемый период
--, ADDRI varchar2(256)  -- адрес для информирования(отменен с 2013г)
, EMPTYPE emptype_tp   -- код категории плательщика: НР, ВЖНР, ООИ, ...
, CONTYPE varchar2(6)  -- тип договора: ТРУ-ТРУДОВОЙ, ГРА-ГРАЖДАНСКО-ПРАВОВОЙ
, ISUM money_tp        -- страховая часть взносов или ОПС 2014г (начислено)
, FSUM money_tp        -- накопительная часть взносов (начислено)
, CISUM money_tp       -- страховая часть взносов или ОПС 2014г из корректируемой (уплачено)
, CFSUM money_tp       -- накопительная часть взносов из корректируемой формы (уплачено)
, XID number(2)        -- индекс xml-файла
);
Type hdr_tbl is table of hdr_rc;
-- Записи о периодах работы
Type pd_rc is record
( FID number         -- индекс формы
, NN number(2)       -- порядковый номер записи 1-99
, SDATE  date        -- дата начала периода
, EDATE  date        -- дата окончания 
, TERR   varchar2(8) -- территориальные условия: МКС, РКС, ...
, TERRC number(4,2)  -- территориальный коэффициент (доля ставки ТУ)
, DD varchar2(16)    -- декрет/дети
, OSOB varchar2(16)  -- особые условия
, OSOBL varchar2(24) -- особый список
, VISL varchar2(16)  -- код выслуги
, VISLC number(4,2)  -- коэффициент выслуги
, ISOSN varchar2(16) -- основание
);
Type pd_tb is table of pd_rc index by pls_integer;
Type pd_tbl is table of pd_rc;
-- Записи о доходах, облагаемых базах
Type inc_rc is record
( FID number       -- индекс формы
, PMON number(2)   -- месяц 1-12 (0 - сумма нарастающим итогом с начала года)
, EMPTYPE emptype_tp -- категория застрахованного лица: НР, ВЖНР, ООИ, ...
, SUMT money_tp    -- сумма выплат
...
Type inc_tb is table of inc_rc index by pls_integer;
Type inc_tbl is table of inc_rc;
-- Записи о дополнительных выплатах
Type pmt_rc is record
( FID number         -- индекс формы
, PMON number(2)     -- месяц 1-12 (0 - сумма нарастающим итогом с начала года)
, ACODE varchar2(10) -- код специальной оценки труда
, SUMA1 money_tp     -- сумма выплат пп1...
...
Type pmt_tb is table of pmt_rc index by pls_integer;
Type pmt_tbl is table of pmt_rc;
-- Структура формы
Type form_tp is record 
( hdr hdr_rc           -- заголовок формы
, periods pd_tb        -- таблица записей о стаже (периодах работы)
, incomes inc_tb       -- таблица записей о доходах/базах
, payments pmt_tb      -- таблица записей о дополнительных выплатах
);
-- XML  Пачка форм (файл) :
Type xml_rc is record
( XID number(2)        -- индекс файла (последовательный номер: 1-99)
, XNAME varchar2(256)  -- имя файла
, XPROGRAM varchar2(80)-- программа
, XVERSION varchar2(20)-- версия
, XFCOUNT number(3):=0 -- количество форм в файле 1-200
, XKEY varchar2(32)    --*** ключ группировки форм = FN:FTYPE:CPINDEX:CPYEAR:CONTYPE:EMPTYPE
                       -- '5:ИСХ::::' = СЗВ-РСВ, ИСХОДНАЯ, отчетный период в org
, FN number(1)         -- вид (номер) формы
, FTYPE varchar2(6)    -- тип сведений: ИСХ, КОР, ОТМ
, CPINDEX number(1)    -- индекс (квартал) корректируемого периода (1-4)
, CPYEAR number(4)     -- корректируемый год (2010-2014)
, EMPTYPE emptype_tp   -- код категории плательщика: НР, ВЖНР, ООИ, ...
, CONTYPE varchar2(6)  -- тип договора: 
--                     'ТРУ'-ТРУДОВОЙ, 'ГРА'-ГРАЖДАНСКО-ПРАВОВОЙ
);
Type xml_tbl is table of xml_rc;
-- XML Информация о файле
Type xml_info_tp is record
( ORG org_rc
, XFILE xml_rc
...
-- Инициализация
Procedure init(p_org org_rc);
Procedure init;
-- Сведения об организации и отчетном периоде
Procedure org_set(p_org org_rc);
Function org_get return org_rc;
-- Управление формами
Procedure form_clear(p_frm in out nocopy form_tp);
Function  form_add(p_frm form_tp) return number;
Procedure form_delete(p_fid number);
Function form_get(p_fid number) return form_tp;
Function forms_tbl return hdr_tbl pipelined;
Function periods_tbl(p_fid number:=null) return pd_tbl pipelined;
Function incomes_tbl(p_fid number:=null) return inc_tbl pipelined;
Function payments_tbl(p_fid number:=null) return pmt_tbl pipelined;
-- Сведения о файлах
Function file_get(p_xid number) return xml_rc;
Procedure file_delete(p_xid number);
Function files_tbl return xml_tbl pipelined;
Function file_info_get(p_xml CLOB) return xml_info_tp;
-- Анализ
Function parse(p_xml CLOB) return number;
-- Генерация
Function generate(p_xid number) return CLOB; -- генерировать файл форм
Function generate return CLOB;               -- генерировать расчетную форму РСВ-1
-- Таблица данных разделов РСВ-1 
subType partname_tp is varchar2(32);
Type part_names_tbl is table of partname_tp;
-- Имена разделов данных РСВ-1
part1_name constant partname_tp  :='PART_1';
part21_name constant partname_tp :='PART_2.1-';  -- PART_2.1-3 ,где 3 код_тарифа
part251_name constant partname_tp:='PART_2.5.1';
part252_name constant partname_tp:='PART_2.5.2';
part4_name constant partname_tp  :='PART_4';
-- Имена полей титульной страницы
--fld_orgokved  constant partname_tp:='FLD_ORGOKVED';  -- ОКВЭД организации
fld_empcount constant partname_tp:='FLD_EMPCOUNT';   -- количество ЗЛ
fld_empavg constant partname_tp  :='FLD_EMPAVG';     -- среднесписочная численность
-- Функции работы с данными РСВ-1
Procedure parts_erase;
Function parts_tbl(p_name varchar2:=null, p_row pls_integer:=null) return part_names_tbl pipelined;
Procedure part_value_set
(p_value varchar2, p_part varchar2, p_row pls_integer:=0, p_col pls_integer:=0); 
Procedure part_value_add
(p_value number, p_part varchar2, p_row pls_integer:=0, p_col pls_integer:=0);
...
Function fn2name(p_fn number) return varchar2;
-- XML имя файла вида PFR-700-Y-2014-ORG-123-456-789012-DCK-34567-DPT-000000-DCK-00000.XML
Function file_name_make(p_xid number) return varchar2;
-- XML формат рег. номера ПФР 000-000-000000
Function pfn2num(p_fn varchar2) return number;
Function num2pfn(p_fn number) return varchar2;
-- XML формат СНИЛС 000-000-000 00 или 000-000-000-00
Function snils2num(p_snils varchar2) return number;
Function num2snils(p_snils number) return varchar2; -- возвращает 000-000-000 00
-- XML код квартала: 1,2,3,4,6,9,0
Function pindex2x(p_index number, p_year number:=null) return number;
Function x2pindex(p_quart number, p_year number:=null) return number;
-- XML формат дат DD.MM.YYYY
Function x2date(p_date varchar2) return date;
...
Function x2money(p_money varchar2) return number; 
Function money2x(p_money number) return varchar2; 
-- XML кодировка файла windows-1251 
Function clob2blob(p_xmlc in CLOB, p_charset varchar2:=null) return BLOB;
Function blob2clob(p_xmlb in BLOB, p_charset varchar2:=null) return CLOB;
end;
/
CREATE OR REPLACE PACKAGE BODY  "PF_XML" is
-- Итоги по XML-файлу
Type xtotal_rc is record
( xname varchar2(128)
, fcount number(3):=0
, isum money_tp:=0
, fsum money_tp:=0
...
-- 
Type form_tb is table of form_tp index by pls_integer;
Type xml_tb is table of xml_rc index by pls_integer;
-- Пакет форм/файлов
Type pack_tp is record
( org org_rc     -- плательщик
, forms form_tb  -- список форм
, files xml_tb   -- список XML-файлов
);
-- Tаблица разделов данных РСВ-1 
Type cols_tp is table of varchar2(128) index by pls_integer; -- индекс=номер колонки
Type rows_tp is table of cols_tp index by pls_integer;       -- индекс=код строки
Type parts_tp is table of rows_tp index by partname_tp;     -- индекс=имя раздела-тарифа
-- Аббревиатуры
Type abbr_tb is table of varchar2(50) index by varchar2(10);
c_abbr abbr_tb;
-- Переменные сессии
v_pack pack_tp;   -- пакет форм/файлов
v_parts parts_tp; -- таблица разделов данных РСВ-1 
...
-- Получить индекс периода (1,2,3,4,0) из кода квартала XML (1,2,3,4,6,9,0) 
Function x2pindex(p_quart number, p_year number:=null) return number
is
  l_index number;
begin
...
begin
  ...
  return 'PFR-700-Y-'||trim(to_char(v_pack.org.pyear))||
    '-ORG-'||num2pfn(v_pack.org.opfn)||
    '-DCK-'||trim(to_char(v_pack.org.sdeck+p_xid,'00000'))||'-DPT-000000-DCK-00000.XML';
end;
-- PARTS Данные разделов РСВ-1. Очистить расчетную таблицу РСВ
Procedure parts_erase
as
l_pname varchar2(32):=v_parts.first;
begin
  if v_parts.count=0 then return; end if;
  loop
    continue when v_parts(l_pname).count=0;
...
( p_name varchar2:=null
, p_row pls_integer:=null
) return part_names_tbl pipelined
is
l_val varchar(32);
begin
  if p_name is null then
...
begin
  ...
  if p_frm.hdr.ftype <> 'ИСХ' then
    ...
    l_frm.hdr.cpyear:=null;
    l_frm.hdr.cpindex:=null;
  end if;
  l_frm.hdr.fn := case p_frm.hdr.cpyear
      when 2011 then '1'
      when 2012 then '1'
      when 2013 then '4'
      else '5' end;
  l_xkey:=l_frm.hdr.fn|| ':' ||
    l_frm.hdr.ftype   || ':' ||
    l_frm.hdr.cpindex || ':' || l_frm.hdr.cpyear || ':' ||
    l_frm.hdr.contype || ':' || l_frm.hdr.emptype;
-- определим id формы
  l_fid:=nvl(l_frm.hdr.fid
      , case when v_pack.forms.count=0 then 1 else v_pack.forms.last+1 end );
-- удалим форму, если существует
  form_delete(l_fid);
-- получить id файла по ключу
...
-- нет свободных файлов - добавим      
    ...
    v_pack.files(l_xid).xid:=l_xid;
    v_pack.files(l_xid).xkey:=l_xkey;
    v_pack.files(l_xid).xname:=file_name_make(l_xid);
    v_pack.files(l_xid).fn:=l_frm.hdr.fn;
    v_pack.files(l_xid).ftype:=l_frm.hdr.ftype;
    v_pack.files(l_xid).cpindex:=l_frm.hdr.cpindex;
    v_pack.files(l_xid).cpyear:=l_frm.hdr.cpyear;
    v_pack.files(l_xid).contype:=l_frm.hdr.contype;
    v_pack.files(l_xid).emptype:=l_frm.hdr.emptype;
  end if;
-- увеличим счетчик форм файла
  v_pack.files(l_xid).xfcount:=v_pack.files(l_xid).xfcount+1;
  v_pack.forms(l_fid):=l_frm;
  v_pack.forms(l_fid).hdr.xid:=l_xid;
  v_pack.forms(l_fid).hdr.fid:=l_fid;
  form_clear(l_frm);
  return l_fid;
end;
-- FORM Таблица заголовков форм
...
--dbms_output.put_line(p_xml.getstringval());
  ...
  loop
    part_value_set(nvl(xt.empc0,xt.isum0),part21_name||p_tcode,xt.scode,3);
    part_value_set(nvl(xt.empc1,xt.isum1),part21_name||p_tcode,xt.scode,4);
    part_value_set(nvl(xt.empc2,xt.isum2),part21_name||p_tcode,xt.scode,5);
    part_value_set(nvl(xt.empc3,xt.isum3),part21_name||p_tcode,xt.scode,6);
  end loop;
  return;
end;
-- Анализ РСВ-1 Раздел 2.5
...
-- Анализ РСВ-1 2014г
Function rsv_parse(p_xml xmltype) return number
as
l_xml xmltype;
begin
  parts_erase;
  select column_value into l_xml
    from XMLTable(
      '/ФайлПФР/ПачкаВходящихДокументов/РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014' passing p_xml);
  select empcount, empavg
--    , okved
    into  v_parts(fld_empcount)(0)(0), v_parts(fld_empavg)(0)(0)
--      , v_parts(fld_orgokved)(0)(0)
...
--      , "OKVED" varchar2(16) path 'КодПоОКВЭД'
  ...
  for xt in
    ( select tcode, ixml, mxml
        from XMLTable('*/Раздел2РасчетПоТарифуИдопТарифу/Раздел_2_1' passing l_xml
          columns "TCODE" number  path 'КодТарифа'
         , "IXML" xmltype path 'НаОбязательноеПенсионноеСтрахование2014'
         , "MXML" xmltype path 'НаОбязательноеМедицинскоеСтрахование'
        ) )
  loop
    rsv21_parse(xt.ixml,xt.tcode);
...
-- Анализ. Периоды работы.
Function pd_parse(p_xdoc xmltype) return pd_tb
as
  l_pdtb pd_tb;
begin
  for xt in 
  ...
  loop
    ...
    select nn, x2date(sdatec), x2date(edatec)
        ...
        , visl, x2money(vislcc)
      into l_pdtb(xt.i).nn, l_pdtb(xt.i).sdate, l_pdtb(xt.i).edate
        , l_pdtb(xt.i).terr, l_pdtb(xt.i).terrc
        , l_pdtb(xt.i).osob, l_pdtb(xt.i).osobl
        , l_pdtb(xt.i).isosn, l_pdtb(xt.i).dd
        , l_pdtb(xt.i).visl, l_pdtb(xt.i).vislc
      from XMLTable('/' passing xt.x
        columns "NN" number path 'НомерСтроки'
        , "SDATEC" varchar2(10) path 'ДатаНачалаПериода'
        , "EDATEC" varchar2(10) path 'ДатаКонцаПериода'
        , "TERR"  varchar2(10) path 'ЛьготныйСтаж/ОсобенностиУчета/ТерриториальныеУсловия/ОснованиеТУ'
        , "TERRCC" varchar2(10) path 'ЛьготныйСтаж/ОсобенностиУчета/ТерриториальныеУсловия/Коэффициент'
        , "OSOB" varchar2(100) path 'ЛьготныйСтаж/ОсобенностиУчета/ОсобыеУсловияТруда/ОснованиеОУТ'
        , "OSOBL" varchar2(100) path 'ЛьготныйСтаж/ОсобенностиУчета/ОсобыеУсловияТруда/ПозицияСписка'
        , "ISOSN" varchar2(100) path 'ЛьготныйСтаж/ОсобенностиУчета/ИсчисляемыйСтаж/ОснованиеИС'
        , "DD" varchar2(100) path 'ЛьготныйСтаж/ОсобенностиУчета/ДекретДети'
        , "VISL" varchar2(100) path 'ЛьготныйСтаж/ОсобенностиУчета/ВыслугаЛет/ОснованиеВЛ'
        , "VISLCC" varchar2(10) path 'ЛьготныйСтаж/ОсобенностиУчета/ВыслугаЛет/ДоляСтавки'
      );
  end loop;
  return l_pdtb;
exception
...
-- Анализ. Доходы и базы.
Function inc_parse(p_xdoc xmltype) return inc_tb
as
  l_intb inc_tb;
begin
  for xt in
...
-- Анализ. Выплаты.
Function pmt_parse(p_xdoc xmltype) return pmt_tb
as
l_pmtb pmt_tb;
begin
  for xt in
...
-- Анализ. Заголовок формы.
Function hdr_parse(p_xdoc xmltype) return hdr_rc
as
l_hdr hdr_rc;
begin
  select substr(ftype,1,3), lname, fname, sname, snils2num(snilsc)
...
-- Имя файла и программы генерации
  select trim(xname), trim(xprogram), trim(xversion)
    into l_info.xfile.xname, l_info.xfile.xprogram, l_info.xfile.xversion
    from XMLTable('/ФайлПФР' passing p_xml
        columns "XNAME" varchar2(200) PATH 'ИмяФайла'
       ,"XPROGRAM" varchar2(200) PATH 'ЗаголовокФайла/ПрограммаПодготовкиДанных/НазваниеПрограммы'
       ,"XVERSION" varchar2(200) PATH 'ЗаголовокФайла/ПрограммаПодготовкиДанных/Версия'
...
-- Вид (номер) формы
  ...
  l_info.xfile.fn:=dtype2fn(l_rootname,c_xhds);
-- Опись пачки документов
  select substr(nvl(nvl(onames,oname),oname9),1,50), pf_xml.pfn2num(nvl(opfn,opfn9))
    , nvl(oinn,oinn9), nvl(okpp,okpp9)
    , nvl(nvl(pcode,pcode9),0), nvl(nvl(nvl(pyear,pyear3),pyear9),0), deckn
    , okved9
    , xfcount, substr(ftype,1,3), cpcode, nvl(cpyear,cpyear3)
    , substr(contype,1,3), emptype
    into l_info.org.oname, l_info.org.opfn, l_info.org.oinn, l_info.org.okpp
      , l_info.org.pindex, l_info.org.pyear, l_info.org.sdeck
      , l_info.org.okved
      , l_info.xfile.xfcount, l_info.xfile.ftype, l_info.xfile.cpindex, l_info.xfile.cpyear
      , l_info.xfile.contype, l_info.xfile.emptype
    from XMLTable('/ФайлПФР/ПачкаВходящихДокументов/*[1]' passing p_xml
...
as
begin
  return xinfo_parse(xmltype(p_xml));
end;
-- Анализ XML-файла 
Function parse(p_xml clob) return number
as
...
begin
  ...
  for ft in 
    ( select rownum i, st.* 
        from
        ( select cpindex, cpyear
           , sum(nvl(cisum,0)-nvl(isum,0)) disum, sum(nvl(cfsum,0)-nvl(fsum,0)) dfsum  
           , decode(grouping_id(cpyear,cpindex),3,'ИТОГ','МЕСЦ') stype
          from table(forms_tbl) 
          where snils=v_pack.forms(p_fid).hdr.snils and ftype in ('КОР','ОТМ') 
          group by rollup(cpyear, cpindex)
          having grouping_id(cpyear,cpindex) in (0,3)
          order by cpyear, cpindex ) st )
  loop
    l_crc:=xnode('СведенияОкорректировках',
      xnode('НомерСтроки',ft.i)||
      xnode('ТипСтроки',ft.stype)||
      case when ft.stype = 'ИТОГ'
        then ''
        else xnode('Квартал',pindex2x(ft.cpindex,ft.cpyear))||
          xnode('Год',ft.cpyear)
      end ||
      case when ft.cpyear >= 2014
        then xnode('СуммаДоначисленныхВзносовОПС',money2x(ft.disum))
        else
          xnode('СуммаДоначисленныхВзносовНаСтраховую',money2x(ft.disum))||
          xnode('СуммаДоначисленныхВзносовНаНакопительную',money2x(ft.dfsum))
      end
    );
    dbms_lob.copy(p_cxml,l_crc,dbms_lob.getlength(l_crc),dbms_lob.getlength(p_cxml)+1,1);
  end loop;
...
*/
  for pt in
    (select rownum i, tb.* from table (payments_tbl(p_fid)) tb
       ...
       order by acode, nvl(pmon,0))
  loop
    if l_acode <> nvl(pt.acode,'acode') then
      l_acode:=pt.acode;
      l_ai:=l_ai+10;
    end if;
    l_crc:=''||
...
begin
  if v_pack.forms(p_fid).hdr.fn < fn_szv4 then return; end if;
  if v_pack.forms(p_fid).incomes.count = 0 then
    v_pack.forms(p_fid).incomes(1):=l_inrc; -- добавить пустую запись
--    dbms_lob.append(p_cxml,
--      case v_pack.forms(p_fid).hdr.fn
--        when fn_szvr then '<СведенияОсуммеВыплатИвознагражденийВпользуЗЛ/>' -- СЗВ-РСВ
--        else '<СуммаВыплатИвознагражденийВпользуЗЛ/>' -- СЗВ-6-4
--      end );
--    return;
  end if;
-- выбрать месяцы периода, группировать по emptype
  for it in 
    (select rownum i, tb.* from table (incomes_tbl(p_fid)) tb
       ...
       order by emptype, nvl(pmon,0))
  loop
    if l_emptype <> nvl(it.emptype,'emptype') then
      l_emptype:=it.emptype;
      l_ei:=l_ei+10;
    end if;
    case v_pack.forms(p_fid).hdr.fn
      when fn_szv4 then
      ...
      when fn_szvr then
        l_crc:=xnode('СведенияОсуммеВыплатИвознагражденийВпользуЗЛ',
          ...
          xnode('КодСтроки',400+l_ei+(it.pmon-(l_emon-3)),nvl(it.pmon,0)>0) ||
          xnode('КодКатегории',it.emptype) ||
          xnode('СуммаВыплатИныхВознаграждений',money2x(it.sumt),it.sumt is not null) ||
          xnode('НеПревышающиеВсего',money2x(it.sumb),it.sumb is not null)||
          xnode('НеПревышающиеПоДоговорам',money2x(it.sumbg),it.sumbg is not null)||
          xnode('ПревышающиеПредельную',money2x(it.sumo),it.sumo is not null)
        );
      else null;
    end case;
    dbms_lob.copy(p_cxml,l_crc,dbms_lob.getlength(l_crc),dbms_lob.getlength(p_cxml)+1,1);
...
begin
  ...
  for i in v_pack.forms(p_fid).periods.first..v_pack.forms(p_fid).periods.last loop
    ...
    l_crc:=xnode('СтажевыйПериод',
      ...
      xnode('ДатаКонцаПериода',date2x(l_pdrc.edate))||
      case 
        when trim(l_pdrc.terr||l_pdrc.dd||l_pdrc.visl||l_pdrc.isosn||l_pdrc.osob) is not null
        then
          xnode('КоличествоЛьготныхСоставляющих',1) ||
          xnode('ЛьготныйСтаж',
            xnode('НомерСтроки',1)||
            xnode('ОсобенностиУчета',
              xnode('ТерриториальныеУсловия',
                xnode('ОснованиеТУ',l_pdrc.terr)||
                xnode('Коэффициент',money2x(nvl(l_pdrc.terrc,0)))
              , l_pdrc.terr is not null)||
              xnode('ОсобыеУсловияТруда',
                xnode('ОснованиеОУТ',l_pdrc.osob)||
                xnode('ПозицияСписка',l_pdrc.osobl)
              )||    
              xnode('ИсчисляемыйСтаж',
                xnode('ОснованиеИС',l_pdrc.isosn)
              )||
              xnode('ДекретДети',l_pdrc.dd)||
              xnode('ВыслугаЛет',
                xnode('ОснованиеВЛ',l_pdrc.visl)||
                xnode('ДоляСтавки',money2x(l_pdrc.vislc))
              ,l_pdrc.visl is not null)
            )
          )
        else ''  
      end
...
begin
  dbms_lob.createtemporary(l_cxml, true, dbms_lob.session);  
  for ft in (select rownum i, ftbl.* from table(pf_xml.forms_tbl()) ftbl where xid=p_xid)
  loop
    l_form:=v_pack.forms(ft.fid);
    l_cxml:='<'||c_xdts(l_form.hdr.fn)||'>'||
        xnode('НомерВпачке',ft.i+1)||
...
-- Генерация. Расчет сумм по формам файла
Function xtotal_calc(p_xid number) return xtotal_rc
as
l_xtt xtotal_rc;
l_emon number:=nvl(v_pack.files(p_xid).cpindex,v_pack.org.pindex)*3;
l_smon number:=l_emon-2;
...
/*
  select '' xname
    ...
    from 
      (select fid, isum, fsum, nvl(cisum, isum) cisum, nvl(cfsum, fsum) cfsum 
         from table(pf_xml.forms_tbl) where xid=p_xid) ft
      left join 
      (select fid
         , sum(nvl(sumt,0)) sumt, sum(nvl(sumb,0)) sumb, sum(nvl(sumo,0)) sumo
         from table(pf_xml.incomes_tbl) where pmon between l_smon and l_emon
         group by fid) it 
      on ft.fid=it.fid;
*/  
  for ft in
    (select * from table(pf_xml.forms_tbl) where xid=p_xid)
  loop
    l_xtt.fcount:=l_xtt.fcount+1;
    l_xtt.isum:=l_xtt.isum+nvl(ft.isum,0);
    l_xtt.fsum:=l_xtt.fsum+nvl(ft.fsum,0);
...
-- Генерация XML-файла    
Function generate(p_xid number) return clob
as
l_cxml CLOB;
l_fn number;
l_xttl xtotal_rc;
begin
  ...
  if l_fn=fn_szv3 then raise_error(4); end if; -- формы СЗВ-6-3 не генерируются
  l_xttl:=xtotal_calc(p_xid);
  dbms_lob.createtemporary(l_cxml, true, dbms_lob.call);
  if l_xttl.fcount=0 then return l_cxml; end if;
  l_xttl.xname:=file_name_make(p_xid);
  l_cxml:='<?xml version="1.0" encoding="Windows-1251" ?><ФайлПФР>'||
    xnode('ИмяФайла', l_xttl.xname)||
    xnode('ЗаголовокФайла',
        xnode('ВерсияФормата','07.00')||
        xnode('ТипФайла','ВНЕШНИЙ')||
        xnode('ПрограммаПодготовкиДанных',
            xnode('НазваниеПрограммы',upper(c_program))||
            xnode('Версия',c_version)
        )||
        xnode('ИсточникДанных','СТРАХОВАТЕЛЬ')
    )||'<ПачкаВходящихДокументов Окружение="В составе файла" Стадия="До обработки">'||
    xnode(c_xhds(l_fn),
        ...
        xnode('СоставительПачки', org2x)||
        xnode('НомерПачки',
            xnode('Основной',trim(to_char(v_pack.org.sdeck+p_xid,'00000')))
        )||
        xnode('СоставДокументов',
            xnode('Количество',1)||
            xnode('НаличиеДокументов',
...
Procedure rsv_part4_append(p_cxml in out nocopy CLOB)
as
l_p4tag varchar2(200):='Раздел4СуммыДоначисленныхСтраховыхВзносов2014';
Function part4_common(p_row number) return varchar2
as
begin
  return
    xnode('СуммаДоначисленныхВзносовОПС2014всего', money2x(part_value_get(part4_name,p_row,6)))||
    xnode('СуммаДоначисленныхВзносовОПС2014превыщающие', money2x(part_value_get(part4_name,p_row,7)))||
    xnode('СуммаДоначисленныхВзносовНаСтраховуюВсего', money2x(part_value_get(part4_name,p_row,8)))||
    xnode('СуммаДоначисленныхВзносовНаСтраховуюПревышающие', money2x(part_value_get(part4_name,p_row,9)))||
    xnode('СуммаДоначисленныхВзносовНаНакопительную', money2x(part_value_get(part4_name,p_row,10)))||
    xnode('СтраховыхДоначисленныхВзносовПоДопТарифуЧ1', money2x(part_value_get(part4_name,p_row,11)))||
    xnode('СтраховыхДоначисленныхВзносовПоДопТарифуЧ2', money2x(part_value_get(part4_name,p_row,12)))||
    xnode('СтраховыхДоначисленныхВзносовПоДопТарифуЧ2_1', money2x(part_value_get(part4_name,p_row,13)))||
    xnode('СтраховыеВзносыОМС', money2x(part_value_get(part4_name,p_row,14)));
end;
begin
  if not v_parts.exists(part4_name) then return; end if;
  dbms_lob.append(p_cxml,'<'||l_p4tag||'>');
  for i in 1..v_parts(part4_name).last loop
    continue when not v_parts(part4_name).exists(i);
    dbms_lob.append(p_cxml, xnode('СуммаДоначисленныхВзносовЗаПериод',
        xnode('НомерПП',part_value_get(part4_name,i,1))||
        xnode('ОснованиеДляДоначисления',part_value_get(part4_name,i,2))||
        xnode('КодОснованияДляДопТарифа',part_value_get(part4_name,i,3))||
        xnode('Год',part_value_get(part4_name,i,4))||
  ...
  dbms_lob.append(p_cxml, xnode('ИтогоДоначислено', part4_common(0))||
      '</'||l_p4tag||'>');
  return;
end;
-- Генерация РСВ-1. Разделы 2.5.1, 2.5.2   
  ...
  for i in 1..v_parts(part251_name).last loop
    l_p25c:=l_p25c||
         xnode('СведенияОпачкеИсходных',
              xnode('НомерПП',part_value_get(part251_name,i,1))||
              xnode('БазаДляНачисленияСтраховыхВзносовНеПревышающаяПредельную'
                  ,money2x(part_value_get(part251_name,i,2))||
                  xnode('СтраховыхВзносовОПС',money2x(part_value_get(part251_name,i,3)))||
                  xnode('КоличествоЗЛвПачке',part_value_get(part251_name,i,4))||
                  xnode('ИмяФайла',part_value_get(part251_name,i,5))));
  end loop;
-- итоги в строке с номером 0
  dbms_lob.append(p_cxml,
    xnode('ПереченьПачекИсходныхСведенийПУ',
        ...
        xnode('ИтогоСведенияПоПачкам',
            xnode('БазаДляНачисленияСтраховыхВзносовНеПревышающаяПредельную'
                , money2x(part_value_get(part251_name,0,2)))||
            xnode('СтраховыхВзносовОПС', money2x(nvl(part_value_get(part251_name,0,3),0)))||
            xnode('КоличествоЗЛвПачке', part_value_get(part251_name,0,4)))
  ));
  l_p25c:='';
  for i in 1..v_parts(part252_name).last loop
    l_p25c:=l_p25c||
          xnode('СведенияОпачкеКорректирующих',
            ...
            xnode('КорректируемыйОтчетныйПериод',
                ...
                xnode('Год',part_value_get(part252_name,i,3)))||
            xnode('ДоначисленоСтраховыхВзносовОПС',
                money2x(nvl(part_value_get(part252_name,i,4),0)))||
            xnode('ДоначисленоНаСтраховуюЧасть',
                money2x(nvl(part_value_get(part252_name,i,5),0)))||
            xnode('ДоначисленоНаНакопительнуюЧасть',
                money2x(nvl(part_value_get(part252_name,i,6),0)))||
            xnode('КоличествоЗЛвПачке',part_value_get(part252_name,i,7))||
            xnode('ИмяФайла',trim(part_value_get(part252_name,i,8))));
  end loop;
  dbms_lob.append(p_cxml,
      xnode('ПереченьПачекКорректирующихСведенийПУ',
        ...
        xnode('ИтогоСведенияПоПачкамКорректирующих',
            xnode('ДоначисленоСтраховыхВзносовОПС'
                , money2x(nvl(part_value_get(part252_name,0,4),0)))||
            xnode('ДоначисленоНаСтраховуюЧасть'
                , money2x(nvl(part_value_get(part252_name,0,5),0)))||
            xnode('ДоначисленоНаНакопительнуюЧасть'
                , money2x(nvl(part_value_get(part252_name,0,6),0)))||
            xnode('КоличествоЗЛвПачке',part_value_get(part252_name,0,7)))
  ));
  dbms_lob.append(p_cxml,'</Раздел_2_5>');
  return;
...
begin
  return
    xnode('КодСтроки',p_scode)||
    case when p_scode in (207,208) then
       xnode('КоличествоЗЛ_Всего',part_value_get(p_part,p_scode,3))||
       xnode('КоличествоЗЛ_1месяц',part_value_get(p_part,p_scode,4))||
       xnode('КоличествоЗЛ_2месяц',part_value_get(p_part,p_scode,5))||
       xnode('КоличествоЗЛ_3месяц',part_value_get(p_part,p_scode,6))
       ...
       xnode('РасчетСумм',
          xnode('СуммаВсегоСначалаРасчетногоПериода',money2x(part_value_get(p_part,p_scode,3)))||
          xnode('СуммаПоследние1месяц',money2x(part_value_get(p_part,p_scode,4)))||
          xnode('СуммаПоследние2месяц',money2x(part_value_get(p_part,p_scode,5)))||
          xnode('СуммаПоследние3месяц',money2x(part_value_get(p_part,p_scode,6)))
       )
    end;
end;
begin
...
--dbms_output.put_line(i);
    ...
    dbms_lob.append(p_cxml,
      xnode('КодТарифа'
          ,substr('0'||regexp_replace(pt.pname,part21_name||'(*)','\1'),-2,2))||
      xnode('НаОбязательноеПенсионноеСтрахование2014',
          xnode('СуммаВыплатИвознагражденийОПС',part21_blk(200,pt.pname))||
          xnode('НеПодлежащиеОбложениюОПС',part21_blk(201,pt.pname))||
          xnode('СуммаРасходовПринимаемыхКвычетуОПС',part21_blk(202,pt.pname))||
          xnode('ПревышающиеПредельнуюВеличинуБазыОПС',part21_blk(203,pt.pname))||
...
begin
  return  
    xnode('КодСтроки',p_scode)||
    xnode('СтраховыеВзносыОПС',money2x(part_value_get(part1_name,p_scode,3)))||
    case when p_scode not in (110,111,112,113,114)
      then
        xnode('ОПСстраховаяЧасть',money2x(part_value_get(part1_name,p_scode,4)))||
        xnode('ОПСнакопительнаяЧасть',money2x(part_value_get(part1_name,p_scode,5))
            ,p_scode<>121)
      else ''
    end ||
    xnode('ВзносыПоДопТарифу1',money2x(part_value_get(part1_name,p_scode,6)),p_scode<>121)||
    xnode('ВзносыПоДопТарифу2_18',money2x(part_value_get(part1_name,p_scode,7)),p_scode<>121)||
    xnode('СтраховыеВзносыОМС',money2x(part_value_get(part1_name,p_scode,8)),p_scode<>121);
end;
begin
  dbms_lob.append(p_cxml,'<'||l_p1tag||'>');
  dbms_lob.append(p_cxml,
...
as
l_cxml CLOB;
l_rtag constant varchar2(200):='РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014';
l_p2tag constant varchar2(200):='Раздел2РасчетПоТарифуИдопТарифу';
begin
  dbms_lob.createtemporary(l_cxml, true, dbms_lob.call);
  if v_pack.files.count=0 then return l_cxml; end if;
...
-- Преобразования BLOB <-> CLOB (XML файлы ПФР используют кодировку windows1251)
Function blob2clob(p_xmlb BLOB, p_charset varchar2:=null) return CLOB
is
  c_lob_maxsize number := DBMS_LOB.LOBMAXSIZE;
  c_lang_context  integer := DBMS_LOB.DEFAULT_LANG_CTX;
  c_warning       integer := DBMS_LOB.WARN_INCONVERTIBLE_CHAR;
...
begin
  ...
  , dest_offset =>c_start_clob
  , src_offset  =>c_start_blob
  , blob_csid   =>NLS_CHARSET_ID (nvl(p_charset,'CL8MSWIN1251'))
  , lang_context=>c_lang_context
  , warning     =>c_warning
  );
return l_clob;
